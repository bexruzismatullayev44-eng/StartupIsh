import { useEffect, useState } from "react";
import axios from "axios";
import { Worker } from "../../types";
import { CiHeart, CiShoppingCart } from "react-icons/ci";
import { API } from "../../types";
import { Link } from "react-router-dom";
import { useCartContext } from "../../Providers/CartProvider";

const Workers = () => {
  const { addToCart } = useCartContext();
  const [workers, setWorkers] = useState<Worker[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  const getWorkers = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get<Worker[]>(`${API}/workers`);
      setWorkers(data);
    } catch (error) {
      console.error("Ishchilarni yuklashda xatolik yuz berdi:", error);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    getWorkers();
  }, []);

  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-75">
        <span className="text-lg text-gray-500 animate-pulse">Yuklanmoqda...</span>
      </div>
    );
  }

  return (
    <div className="max-w-7xl mx-auto p-4 dark:bg-gray-700!">
      <div className="flex justify-between items-end mb-5">
        <h2 className="m-0 text-2xl text-gray-800 font-semibold dark:text-white!">
          Xodimlar va Mutaxassislar
        </h2>
        <Link
          to="/MainWorker"
          className="relative z-50 no-underline! text-gray-800! hover:text-gray-800! transition-color! dark:text-white!"
        >
          Barchasi
        </Link>
      </div>

      {workers.length === 0 ? (
        <div className="text-center text-gray-500 py-10">
          Hozircha hech qanday ishchi topilmadi.
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 ">
          {workers.slice(0, 8).map((item) => (
            <div key={item.id} className="group flex flex-col border rounded-2xl! dark:border-0! ">
              <div className="relative aspect-square overflow-hidden rounded-2xl! bg-gray-100! dark:bg-gray-800!">
                <img
                  src={item.images?.[0] || "https://via.placeholder.com/150"}
                  alt={item.title}
                  className="h-full w-full dark:text-white!"
                />
                <button className="absolute top-3 right-3 w-8 h-8 bg-white/90 rounded-full! flex items-center justify-center shadow-sm hover:text-red-500  dark:bg-gray-800/80! dark:text-white!">
                  <CiHeart />
                </button>
              </div>
              <div className="mt-3 flex flex-col flex-1">
                <span className="text-[11px] text-gray-400 uppercase font-bold tracking-wide">
                  {item.jobs?.name || "Kategoriya yo'q"}
                </span>
                <Link
                  to={`/Worker/${item.id}`}
                  className="no-underline! text-gray-800! hover:text-gray-800! transition-color!"
                >
                  <h3 className="text-sm font-medium text-gray-800 line-clamp-1 mt-1 dark:text-white!">
                    {item.title}
                  </h3>
                </Link>

                <div className="mt-auto pt-3 flex items-center justify-between">
                  <span className="text-[16px] font-bold text-gray-900 dark:text-white!">
                    {item.price?.toLocaleString()} so'm
                  </span>
                  <button
                    onClick={() => addToCart(item)}
                    className="w-9 h-9 bg-[#222] text-white rounded-full! flex items-center justify-center hover:bg-orange-600 transition-all active:scale-95"
                  >
                    <CiShoppingCart />
                  </button>
                </div>
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  );
};

export default Workers;