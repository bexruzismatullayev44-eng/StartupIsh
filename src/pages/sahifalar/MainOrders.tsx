...
const MainOrders = () => {
      ...
      try {
        ...
        data.forEach((order) => {
          ...
          const myItems = order.orderItems.filter((item: any) => {
            ...
            return (
              workerId &&
              String(workerId).trim() === String(currentUserId).trim()
            );
          });
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900! transition-colors duration-200 text-black dark:text-white!">
        ...
        {loading ? (
          <div className="space-y-4">
            {[1, 2].map((n) => (
              <div
                key={n}
                className="animate-pulse bg-white dark:bg-gray-800! border h-44 rounded-2xl"
              ></div>
            ))}
          </div>
        ) : orders.length > 0 ? (
            ...
            {orders.map((order) => (
              ...
              >
                ...
                <div className="bg-gray-50/70 dark:bg-gray-700/30! px-6 py-4 border-b border-gray-100 dark:border-gray-700/50 flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                      {order.createAt
                        ? new Date(order.createAt).toLocaleDateString("uz-UZ", {
                            year: "numeric",
                            month: "long",
                            day: "numeric",
                            hour: "2-digit",
                ...
                <div className="bg-gray-50/30 dark:bg-gray-800/50! px-6 py-3 border-t border-gray-50 dark:border-gray-700/30! text-sm flex flex-col gap-1">
                  <p className="text-blue-600 dark:text-blue-400 font-bold mb-1 border-b pb-1 border-gray-100 dark:border-gray-700">
                    Sizni yollagan mijoz ma'lumotlari:
                  </p>
                  <span className="text-xs text-gray-500 dark:text-gray-400!">
                    <strong className="text-gray-700 dark:text-gray-300!">
                      Ismi:
                    </strong>{" "}
                    {order.fullName || "Noma'lum"}
                  </span>
                  <span className="text-xs text-gray-500 dark:text-gray-400!">
                    <strong className="text-gray-700 dark:text-gray-300!">
                      Tel raqami:
          ...
          <div className="text-center py-20 bg-white dark:bg-gray-800! rounded-2xl border border-gray-100 dark:border-gray-700! shadow-sm">
            ...
            <p className="text-sm text-gray-400 max-w-sm mx-auto">
              Mijozlar aynan sizni savatga qo'shib xarid qilishganda,
              ma'lumotlar bu yerda toza holatda chiqadi.
            </p>
          </div>
        )}
      </div>
